...
async function registerToken(
  ...
  if (!token) return;
  try {
    // The admin fetch client's base URL already carries the `/v1` version
    // segment, so this resolves to the same `/api/v1/devices` endpoint the
    // client app registers against.
    await api.post<{ registered: boolean }>('/devices', { token, platform });
